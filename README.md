...
A Python toolkit for working with and efficiently storing AlphaFold3 results. Contains both a CLI for importing/exporting results to a highly efficient foldkit storage format, and a python API for easy access for metrics of folded complexes.

## Installation
`pip install foldkit`

For bash autocompletion of the foldkit CLI, run:

`activate-global-python-argcomplete`

or 

`activate-global-python-argcomplete --user` if on a shared workspace.

You may then need to restart your shell for it to be activated.


## Using foldkit
There are two main use cases for this package:

(1) **Convenient python interface for accessing AlphaFold3 prediction confidence metrics.** 

This is particularly useful for pulling out inter-chain metrics from predicted protein complexes, as they have been shown to be predictive of binding and are useful criteria for protein design and specificity predictions.

(2) **Efficient storage and retrieval of AlphaFold3 results.**

 The default JSON formats for AF3 confidence results are large, and can take up a lot of unnecessary space. Foldkit has a CLI for exporting the AF3 confidence JSONs to space-efficient .npz files, removing other unnecessary files, and copying over the rest. These .npz files can also be used as alternative inputs to the python interface in (1) for easy use. 
